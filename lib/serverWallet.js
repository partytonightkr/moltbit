...
// wei (decimal/bigint) → 0x-hex for the tx `value` field
const weiHex = (wei) => "0x" + BigInt(wei || 0).toString(16);
 ...
 */
export async function openVenuePosition({ env, adapterAddress, walletId, executionFee = 0, ...params }) {
  if (!LIVE || !adapterAddress) return { txHash: mockHash(), mode: "mock" };
  const data = encodeAdapterOpen(params);
  const txHash = await sendVaultTx({ env, walletId, to: adapterAddress, data, value: weiHex(executionFee) });
  return { txHash, mode: "live" };
}

/** Close (or partially close) a position on the adapter (adapter.closeTrade). Mock-safe. */
export async function closeVenuePosition({ env, adapterAddress, walletId, executionFee = 0, ...params }) {
  if (!LIVE || !adapterAddress) return { txHash: mockHash(), mode: "mock" };
