...
export const ADAPTER_ABI = [
  ...
  { type: "function", name: "closeTrade", stateMutability: "payable", inputs: [
    ...
    { name: "collateralToClose", type: "uint256" }, { name: "executionFee", type: "uint256" },
  ], outputs: [] },
...
// open a position on the adapter. Human inputs in, on-chain scaling out.
export function encodeAdapterOpen({ pairIndex, buy, marginUsdc, openPrice = 0, leverage = 1, tp = 0, sl = 0, orderType = 0, slippagePct = 1, executionFee = 0 }) {
  return encodeFunctionData({ abi: ADAPTER_ABI, functionName: "openTrade", args: [
    ...
    Number(orderType), e10(slippagePct), BigInt(executionFee),
  ] });
}
export function encodeAdapterClose({ pairIndex, index, collateralToCloseUsdc, executionFee = 0 }) {
  return encodeFunctionData({ abi: ADAPTER_ABI, functionName: "closeTrade", args: [
    BigInt(pairIndex), BigInt(index), usdc6(collateralToCloseUsdc), BigInt(executionFee),
  ] });
}
