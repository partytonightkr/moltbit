...
// Map an instrument/market string to a policy category. Accepts either a bare
// category ("perps"/"spot"/"options"/"fx") or an instrument symbol the skill/docs
// use ("ETH-PERP", "BTC", "ETH-OPT", "EUR/USD"). Bare symbols default to spot.
export function marketClass(market) {
  const m = String(market || "").toLowerCase();
  if (m === "perps" || m === "spot" || m === "options" || m === "fx") return m;
  if (m.includes("perp") || m.includes("fut")) return "perps";
  if (m.includes("opt") || m.includes("call") || m.includes("put") || /-[cp]\d/.test(m)) return "options";
  if (m.includes("fx") || m.includes("/") || /\b(eur|jpy|gbp|chf|aud|cad)\b/.test(m)) return "fx";
...
export function checkOrder(order, policy, state) {
  ...
  // 1. market allowed — classify the instrument (e.g. "ETH-PERP") into a category
  const cls = marketClass(o.market);
  if (!p.markets || !p.markets[cls]) {
    return deny("MARKET_BLOCKED", `market '${o.market}' (${cls}) not permitted`);
  }
