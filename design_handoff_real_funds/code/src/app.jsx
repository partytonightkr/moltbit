...
import { sendUsdc, depositAddressFor, isAddr, explorerTx } from './chain.js';
...
function DepositModal({ ctx, mode, env, toast, onClose }) {
  ...
  const doDeposit = async () => {
    // Real on-chain USDC deposit when signed in with a real wallet and a destination is set.
    if (real && isAddr(depositAddr)) {
      try {
        setBusy(true);
        const hash = await sendUsdc(env || "test", auth.wallet, depositAddr, amt);
    ...
    if (real && !isAddr(depositAddr)) toast?.("Set VITE_DEPOSIT_ADDRESS to enable real deposits");
        ...
        {!strat && isAgent ? (
          ...
          <div className="modal-body">
            ...
            <div className="modal-breakdown">
              ...
              <div className="mb-row"><span>Settlement</span><span>{real ? (env === "live" ? "Base · USDC" : "Base Sepolia · USDC") : "anytime · T+0"}</span></div>
            </div>
            <button className="modal-go" disabled={busy || amt <= 0} onClick={doDeposit}>
              {busy ? "Confirming…" : isAgent ? "⚡ Allocate $" + amt.toLocaleString() : (real && isAddr(depositAddr) ? "＋ Deposit $" + amt.toLocaleString() + " USDC" : "＋ Deposit $" + amt.toLocaleString())}
            </button>
            <span className="modal-fine">
              {real
                ? (isAddr(depositAddr) ? "A real USDC transfer signed by your wallet. Needs a little ETH for gas." : "Demo: set VITE_DEPOSIT_ADDRESS to route real deposits to a vault.")
...
export default function App() {
  ...
  const [env, setEnv] = useState(() => localStorage.getItem("moltbit_env") || "test");
